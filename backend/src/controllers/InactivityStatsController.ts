...
export const getDashboardStats = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    const [
      ...
      sequelize.query(`
        SELECT AVG(EXTRACT(EPOCH FROM (updated_at - last_interaction_at)) / 60) as "averageMinutes"
...
export const getFilteredExecutions = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    if (sortBy === 'lastActivity') {
      ...
      orderBy.push([
        Sequelize.literal(`EXTRACT(EPOCH FROM (COALESCE(updated_at, NOW()) - created_at)) / 60`),
        sortOrder
      ]);
    ...
    const { count, rows: executions } = await FlowBuilderExecution.findAndCountAll({
      ...
      attributes: {
        include: [
          ...
          [
            Sequelize.literal(`EXTRACT(EPOCH FROM (COALESCE(updated_at, NOW()) - COALESCE(last_interaction_at, created_at))) / 60`),
            'inactiveMinutes'
          ]
        ]
      }
