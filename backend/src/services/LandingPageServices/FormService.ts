...
private async processSubmissionAsync(submissionId: number, formData: any, landingPage: any, form: any, companyId: number) {
  ...
  try {
    ...
    let wbot = await getWbot(whatsapp.id);
    ...
    logger.debug(`Dados de contato preparados: ${JSON.stringify({ name: contactData.name, number: contactData.number, email: contactData.email })}`);

      let contact;

      const normalizedNumber = formData.number.replace(/\D/g, "");
      const chatId = `${normalizedNumber}@s.whatsapp.net`;

      try {
        const [result] = await wbot.onWhatsApp(chatId);
        if (!result || !result.exists) {
          throw new Error(`Number ${chatId} does not exist on WhatsApp`);
        } else {
          // Criar ou atualizar contato
          contact = await CreateOrUpdateContactService(contactData, wbot);
        }
      } catch (error) {
        logger.error(`Error checking WhatsApp number ${chatId}:`, error);
        throw new Error(`Number ${chatId} does not exist on WhatsApp`);
      }
      ...
      let processedNumber = await this.validateAndNormalizeWhatsAppNumber(formData.number, wbot, 'número do contato');

      // Etapa 6: Número validado com sucesso, criar/atualizar contato
      logger.info(`Número ${processedNumber} validado com sucesso no WhatsApp`);
      ...
      logger.debug(`Dados de contato preparados: ${JSON.stringify({
      ...
      try {
        ...
        logger.info(`Contato ID: ${contact.id} criado/atualizado com sucesso para o número ${processedNumber}`);
      } catch (error) {
      ...
      logger.debug(`Iniciando processamento de campos personalizados e tags para contato ID: ${contact.id}`);
      await Promise.all([
        this.processCustomFields(contact.id, formData, companyId),
        this.applyContactTags(contact.id, fullLandingPage.advancedConfig?.contactTags, companyId)
      ]);
      logger.info(`Campos personalizados e tags processados com sucesso para contato ID: ${contact.id}`);
