  ...
  const whatsapp = await Whatsapp.create(
    {
      ...
      maxUseBotQueues,
      timeUseBotQueues: timeUseBotQueues ? parseInt(String(timeUseBotQueues)) : 0,
      expiresTicket,
      expiresInactiveMessage,
      timeInactiveMessage: timeInactiveMessage ? parseInt(String(timeInactiveMessage)) : 0,       
      inactiveMessage,            
      collectiveVacationMessage, 
      collectiveVacationStart: collectiveVacationStart ? new Date(collectiveVacationStart) : null,   
      collectiveVacationEnd: collectiveVacationEnd ? new Date(collectiveVacationEnd) : null,   
      allowGroup: normalizedAllowGroup,
      importOldMessages,
      importRecentMessages,
