...
const TicketsListGroup  = (props) => {
  ...
  useEffect(() => {
    const count = getTicketsArrayByUser(ticketsList, user).filter(ticket => ticket.isGroup).length;
...
<List style={{ paddingTop: 0 }}>
  {getTicketsArrayByUser(ticketsList, user).length === 0 && !loading ? (
    ...
    <>
      {getTicketsArrayByUser(ticketsList, user)
        ...
        .map((ticket) => (
          <Can
            ...
            yes={() => (
              <TicketListItem setTabOpen={setTabOpen} ticket={ticket} key={ticket.id} />
            )}
            no={() => null}
          />
        ))}
