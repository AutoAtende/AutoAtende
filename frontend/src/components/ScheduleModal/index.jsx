...
const validationSchema = Yup.object().shape({
  ...
  recurrence: Yup.object().shape({
    ...
    .when("enabled", {
      is: true,
      then: Yup.string().required(i18n.t("schedules.validation.patternRequired"))
    }),
  endDate: Yup.date()
    .nullable()
    .when("enabled", {
      ...
      then: Yup.date()
        .nullable()
        .min(Yup.ref('$sendAt'), i18n.t("schedules.validation.endDateAfterSendAt"))
        .required(i18n.t("schedules.validation.endDateRequired"))
    })
})
});
...
const ScheduleModal = ({ 
  ...
  // Salvar agendamento
  const handleSaveSchedule = async (values, { setSubmitting, resetForm }) => {
    try {
      setLoading(true);
                ...
                <Grid item xs={12} md={6}>
                  <Paper
                    ...
                    sx={{ 
                      ...
                      borderRadius: 2,
                      borderColor: (
                        touched.recurrence?.pattern && errors.recurrence?.pattern ||
                        touched.recurrence?.endDate && errors.recurrence?.endDate
                      ) ? theme.palette.error.main : undefined
                    }}
                  >
                    <Box 
                    ...
                    <FormControlLabel
                      ...
                      {values.recurrence.enabled && (
                        <Box mt={2} display="flex" flexDirection="column" gap={2}>
                          <FormControl
                            fullWidth
                            error={touched.recurrence?.pattern && Boolean(errors.recurrence?.pattern)}
                          >
                            <InputLabel>
                            ...
                            {touched.recurrence?.pattern && errors.recurrence?.pattern && (
                              <FormHelperText>{errors.recurrence.pattern}</FormHelperText>
                            )}
                          </FormControl>
                          ...
                          <TextField
                            ...
                            error={touched.recurrence?.endDate && Boolean(errors.recurrence?.endDate)}
                            helperText={touched.recurrence?.endDate && errors.recurrence?.endDate}
                            sx={{
                              "& .MuiOutlinedInput-root": {
                                borderRadius: 2
                              }
