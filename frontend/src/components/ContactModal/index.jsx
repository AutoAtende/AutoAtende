        ...
        const loadData = async () => {
            if (open) {
                try {
                    ...
                    // Carregar employers com a estrutura correta do controller
                    const employersResponse = await api.get('/employers', {
                        params: {
                            searchParam: '',
                            page: 0, // O controller usa page 0 como início
                            limit: 999999
                        }
                    });
                    
                    // Carregar positions usando a rota simplified
                    ...
                    // Usando a estrutura correta retornada pelo controller
                    ...
                    if (contactId) {
                        ...
                        if (contactData) {
                            ...
                            if (contactData.employerId) {
                                const employer = employersResponse.data.employers.find(
                                    emp => emp.id === contactData.employerId
                                );
                            ...
                            if (contactData.positionId) {
                                const position = positionsResponse.data.positions.find(
                                    pos => pos.id === contactData.positionId
                                );
