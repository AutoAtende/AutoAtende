...
const sendAlbum = async () => {
  ...
  try {
    ...
    // Enviar múltiplas mensagens com delay
    for (let i = 0; i < albumFiles.length; i++) {
      ...
      await api.post(`/wbotpro/send-message/${selectedWhatsapp}`, {
        jid,
        type: file.type,
...
const sendCarousel = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      toast.error(response.data.message || 'Erro ao enviar carrossel');
    }
  } catch (error) {
...
const sendPaymentRequest = async () => {
  ...
  try {
    ...
    if (response.data.success) {
      ...
      toast.error(response.data.message || 'Erro ao enviar solicitação de pagamento');
    }
  } catch (error) {
    ...
    <Grid container spacing={3}>
      ...
      <Grid item xs={12} md={8}>
          ...
          <CardContent>
            ...
            <Box sx={{ mb: 3 }}>
              ...
              <Button
                ...
                variant="contained"
                onClick={sendMessage}
                disabled={loading || !selectedWhatsapp || !phoneNumber}
                startIcon={<SendIcon />}
                sx={{ mt: 2 }}
              >
                Enviar Mensagem
        ...
        <TestCard>
          <CardContent>
            <Typography variant="h6" gutterBottom>
              Conexões WhatsApp Disponíveis
            </Typography>
            
            {whatsapps.length === 0 ? (
              <Alert severity="info">
                Nenhuma conexão WhatsApp encontrada. Crie uma nova conexão no painel de administração.
              </Alert>
            ) : (
              <List>
                {whatsapps.map((whatsapp) => (
                  <ListItem key={whatsapp.id}>
                    <ListItemText
                      primary={whatsapp.name}
                      secondary={`ID: ${whatsapp.id} • Status: ${whatsapp.status}`}
                    />
                    <ListItemSecondaryAction>
                      <StatusChip
                        size="small"
                        label={whatsapp.status === 'CONNECTED' ? 'Online' : 'Offline'}
                        connected={whatsapp.status === 'CONNECTED'}
                      />
                    </ListItemSecondaryAction>
                  </ListItem>
