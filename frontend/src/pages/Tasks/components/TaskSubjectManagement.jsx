...
const TaskSubjectManagement = ({ open, onClose, onSuccess }) => {
  ...
  }, []);  // Removido 'loading' do array de dependências para evitar loop
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      console.log(`Iniciando ${editingSubjectId ? 'edição' : 'criação'} de assunto: ${name}`);
      
      if (editingSubjectId) {
        await api.put(`/task/subject/${editingSubjectId}`, { 
          name: name.trim(), 
          description: description.trim() 
        });
        toast.success(i18n.t('taskSubjects.subjectUpdated') || 'Assunto atualizado com sucesso');
      } else {
        await api.post('/task/subject', { 
          name: name.trim(), 
          description: description.trim() 
        });
      ...
      // Pausa breve antes de buscar os dados atualizados
      // para garantir que o backend tenha processado as alterações
      setTimeout(() => {
        if (isMounted.current) {
          shouldFetch.current = true;
          fetchSubjects();
      ...
      const errorMessage = errorDetail || i18n.t('taskSubjects.errorSaving') || 'Erro ao salvar assunto';
  ...
  const handleDelete = async (subjectId) => {
    ...
    try {
      ...
      // Atualizar a lista localmente para evitar uma chamada adicional à API
      setSubjects(prev => prev.filter(s => s.id !== subjectId));
      
      // Também atualizar os dados no componente pai
      if (onSuccess && typeof onSuccess === 'function') {
        onSuccess();
      }
    } catch (error) {
      ...
      // Tratamento específico para o caso de assunto sendo usado em tarefas
...
export default TaskSubjectManagement;